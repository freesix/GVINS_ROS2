...
void readParameters(rclcpp::Node &n)
{
    ...
    RCUTILS_LOG_INFO("ROW: %f COL: %f ", ROW, COL);

    ESTIMATE_EXTRINSIC = fsSettings["estimate_extrinsic"];
    if (ESTIMATE_EXTRINSIC == 2)
    {
        RCUTILS_LOG_WARN("have no prior about extrinsic param, calibrate extrinsic param");
    ...
    else 
    {
        ...
        {
            RCUTILS_LOG_WARN(" Optimize extrinsic param around initial guess!");
            EX_CALIB_RESULT_PATH = OUTPUT_DIR + "/extrinsic_parameter.csv";
        }
        if (ESTIMATE_EXTRINSIC == 0)
            RCUTILS_LOG_WARN(" fix extrinsic param ");
    ...
    if (GNSS_ENABLE)
    {
        ...
        gnss_output.close();
        RCLCPP_INFO_STREAM(rclcpp::get_logger("parameters"), "GNSS enabled");
    }
