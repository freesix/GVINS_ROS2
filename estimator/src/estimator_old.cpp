...
void Estimator::processImage(const std::map<int, std::vector<std::pair<int, 
    ...
    imageframe.pre_integration = tmp_pre_integration; // 图像帧的预积分成员
    ...
    if(solver_flag == INITIAL){ // 需要进行初始化
        // RCUTILS_LOG_INFO("need to initial");
        if(frame_count == WINDOW_SIZE){ // 滑动窗口塞满了才进行初始化
            ...
            if(ESTIMATE_EXTRINSIC != 2 && (stamp2Sec(header.stamp)-initial_timestamp) > 0.1){ 
                // RCUTILS_LOG_INFO("befor initialStructure");
                result = initialStructure(); // 初始化
                // RCUTILS_LOG_INFO("after initialStructure");
                initial_timestamp = stamp2Sec(header.stamp);
            }
            if(result){ // 初始化成功
                RCUTILS_LOG_INFO("befor initialization");
            ...
            // RCUTILS_LOG_INFO("silde windows not enough, current frame: %i", frame_count);
        ...
        TicToc t_solve;
        solveOdometry(); // 优化
        RCUTILS_LOG_INFO("solver cost: %f ms", t_solve.toc());  // 优化的时间
        // 失效检测，如果失效则重启VINS系统
        if(failureDetection()){
...
bool Estimator::initialStructure(){
    ...
    // RCUTILS_LOG_INFO("11111111111111111111");
    {
    ...
    // RCUTILS_LOG_INFO("22222222222222222222222");
    ...
    // RCUTILS_LOG_INFO("3333333333333333333333333333");
    ...
    for(int i=0; frame_it != all_image_frame.end(); frame_it++){
        ...
        // RCUTILS_LOG_INFO("all_image_frame: %i", all_image_frame.size());
        ...
        // RCUTILS_LOG_INFO("3333333333555555555555");
        ...
        // RCUTILS_LOG_INFO("33333333333336666666666666");
        ...
        // RCUTILS_LOG_INFO("333333333333777777777777");
        // 求解成功
        cv::Rodrigues(rvec, r);
    ...
    RCUTILS_LOG_INFO("444444444444444444444444");
    if (!visualInitialAlign())
    {
        RCUTILS_LOG_WARN("misalign visual structure with IMU");
        return false;
    }
    RCUTILS_LOG_INFO("5555555555555555555555555");
    return true;
}
    ...
    for(auto &it_per_id : f_manager.feature){ // 遍历滑动窗口中的特征点
        ...
        for(auto &it_per_frame : it_per_id.feature_per_frame){ // 遍历特征点在所有的观测帧
            ...
            else // 不优化时间偏差，只用视觉投影因子
            {
                ...
                // int posei_size = sizeof(para_Pose[imu_i]) / sizeof(para_Pose[imu_i][0]);
                // int posej_size = sizeof(para_Pose[imu_j]) / sizeof(para_Pose[imu_j][0]);
                // int ex_pose_size = sizeof(para_Ex_Pose[0]) / sizeof(para_Ex_Pose[0][0]);
                // int feature_size = (para_Feature[feature_index] != nullptr) ? 
                                    // sizeof(para_Feature[feature_index])/sizeof(para_Feature[feature_index][0]) : 0;
                // RCUTILS_LOG_INFO("para_Posei: %d, para_Posej: %d, para_Ex_Pose: %d, para_Feature: %d",
                                // posei_size, posej_size, ex_pose_size, feature_size);
