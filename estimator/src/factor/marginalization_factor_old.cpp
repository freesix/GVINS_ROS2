...
void MarginalizationInfo::marginalize(){
    ...
    RCUTILS_LOG_INFO("marginalize: pos=%d, m=%d, n=%d", pos, m, n);
    ...
    */
    ...
    RCUTILS_LOG_INFO("pthread_create error1");
    for(int i=0; i<NUM_THREADS; i++){
        ...
        RCUTILS_LOG_INFO("1111111111111");
        int ret = pthread_create(&tids[i], NULL, ThreadsConstructA, (void*)&(threadsstruct[i]));
        RCUTILS_LOG_INFO("2222222222222");
        if(ret != 0){
            RCUTILS_LOG_WARN("pthread_create error");
            // TODO: ros退出
        }
    }
    RCUTILS_LOG_INFO("pthread_create error2");
    for(int i=NUM_THREADS-1; i>=0; i--){
        pthread_join(tids[i], NULL);
        A += threadsstruct[i].A;
        b += threadsstruct[i].b;
    }
    RCUTILS_LOG_INFO("A matrix size: %d×%d, b vector size: %d", A.rows(), A.cols(), b.size());
    ...
    RCUTILS_LOG_INFO("Amm_inv size: %d×%d, saes.eigenvectors size: %d×%d",
        Amm_inv.rows(), Amm_inv.cols(), saes.eigenvectors().rows(), saes.eigenvectors().cols());
    ...
    RCUTILS_LOG_INFO("Amm: %d×%d, Amr: %d×%d, Arm: %d×%d, Arr: %d×%d",
        Amm.rows(), Amm.cols(), Amr.rows(), Amr.cols(),
        Arm.rows(), Arm.cols(), Arr.rows(), Arr.cols());
    RCUTILS_LOG_INFO("bmm: %d, brr: %d", bmm.size(), brr.size());
