...
void img_callback(const sensor_msgs::msg::Image::SharedPtr img_msg)
{
   ...
   if (PUB_THIS_FRAME)
   {
        ...
        RCUTILS_LOG_DEBUG("publish %f, at %f", stamp2Sec(feature_points->header.stamp), rclcpp::Clock().now().seconds());
    ...
    RCUTILS_LOG_INFO("whole feature tracker processing costs: %f", t_r.toc());
}
