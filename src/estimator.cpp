...
Estimator::Estimator() : f_manager{Rs}{
    ...
    for(int i=0; i<WINDOW_SIZE+1; i++){
        pre_integrations[i] = nullptr; // 每一帧的预积分对象初始化为空
    }
    clearState(); // 初始化清零
}
...
void Estimator::processImage(const std::map<int, std::vector<std::pair<int, 
    Eigen::Matrix<double, 7, 1>>>> &image, const double &header){
    LOG(WARNING) << "new image coming -----------------------------------";
    LOG(WARNING) << "Adding feature points " << image.size();
    ...
    LOG(WARNING) << "this frame is ------------" << (marginalization_flag ? "reject" : "accept");
    LOG(WARNING) << (marginalization_flag ? "Non-keyframe" : "Keyframe");
    LOG(WARNING) << "Solving " << frame_count;
    LOG(WARNING) << "number of feature: " << f_manager.getFeatureCount();
    Headers[frame_count] = header;

    ImageFrame imageframe(image, header); // 建立图像帧管理对象
    imageframe.per_integration = tmp_pre_integration; // 图像帧的预积分成员
    all_image_frame.insert(std::make_pair(header, imageframe)); // 包含预积分对象的图像帧插入管理所有图像帧的pair
    ...
    if(ESTIMATE_EXTRINSIC == 2){
        ...
        if(frame_count != 0){
            ...
            if(initial_ex_rotation.CalibrationExRotation(corres, 
                ...
                LOG(WARNING) << "initial extrinsic rotation: "<<std::endl<<calib_ric;
                ric[0] = calib_ric;
                RIC[0] = calib_ric;
    ...
    if(solver_flag == INITIAL){ // 需要进行初始化
        ...
        TicToc t_solve;
        solveOdometry(); // 优化
        LOG(WARNING) << "slover cost time: " << t_solve.toc() << "ms"; // 优化时间
        // 失效检测，如果失效则重启VINS系统
        if(failureDetection()){
...
bool Estimator::initialStructure(){
    ...
    if(!relativePose(relative_R, relative_T, l)){
        LOG(INFO) << "Not enough features or parallax; Move divece around";
        return false;
    }
    ...
    for(int i=0; i<WINDOW_SIZE; i++){
        ...
        if(corres.size() > 20){
            ...
            if(average_parallax*460 > 30 && m_estimator.solveRelativeRT(corres, relative_R, relative_T)){
                l = i;
                LOG(WARNING) << "average_parallax " << average_parallax * 460 << " choose l " << l << " and newest frame to triangulate the whole structure";
                return true;
            }
        }
    }    
...
void Estimator::optimization(){
    ...
    LOG(INFO) << "prepare for cers: " << t_prepare.toc();
    ...
    LOG(INFO) << "Iterations: " << static_cast<int>(summary.iterations.size());
    LOG(INFO) << "solver costs: " << t_solver.toc();
    ...
    if(marginalization_flag == MARGIN_OLD){
        ...
        LOG(INFO) << "Pre marginalization costs: " << t_pre_margin.toc();

        TicToc t_margin;
        marginalization_info->marginalize();
        LOG(INFO) << "marginalization costs: " << t_margin.toc();
                ...
                last_marginalization_parameter_blocks), para_Pose[WINDOW_SIZE - 1]))
        {
            ...
            TicToc t_pre_margin;
            LOG(INFO) << "begin marginalization";
            marginalization_info->preMarginalize();
            LOG(INFO) << "end pre marginalization " << t_pre_margin.toc() << " ms";
