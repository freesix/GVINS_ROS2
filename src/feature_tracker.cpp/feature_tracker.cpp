...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time){
    ...
    if(PUB_THIS_FRAME){
        ...
        TicToc t_m;
        setMask();
        std::cout<<"set mask costs: "<<t_m.toc()<<std::endl; 
        ...
        std::cout<<"detect feature costs: "<<t_t.toc()<<std::endl;

        std::cout<<"add feature begins"<<std::endl;
        TicToc t_a;
        addPoints();
        std::cout<<"add feature costs: "<<t_a.toc()<<std::endl;
    }
...
void FeatureTracker::rejectWithF()
{
    if(forw_pts.size() >= 8){
        ...
        std::cout<<"FM ransac costs: "<<t_f.toc()<<std::endl;
    }
}
...
/**
 * @brief 读取相机内参
 * @param calib_file 包含标定数据的yaml文件路径
*/
void FeatureTracker::readIntrinsicParameter(const std::string &calib_file){
    std::cout<<"reading paramerter of camera "<<calib_file<<std::endl;
