...
/*     if (FISHEYE == 1)
    ...
    CAM_NAMES.push_back(config_path);
    ...
    else 
    {
        ...
        LOG(INFO)<<"Extrinsic_R : "<<std::endl<<RIC[0];
        LOG(INFO)<<"Extrinsic_T : "<<std::endl<<TIC[0].transpose(); 
    } 
    ...
    if (ESTIMATE_TD)
        LOG(INFO) << "Online estimate time offset, initial td: " << TD;
    else
